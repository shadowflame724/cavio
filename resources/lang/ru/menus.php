<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Языковые ресурсы названий менюшек
    |--------------------------------------------------------------------------
    |
    | Следующие языковые ресурсы используются в названиях
    | менюшек всего вашего приложения.
    | Вы можете свободно изменять эти языковые ресурсы в соответствии
    | с требованиями вашего приложения.
    |
    */

    'backend'           => [
        'access'        => [
            'roles' => [
                'all'           => 'Все роли',
                'create'        => 'Создать роль',
                'edit'          => 'Редактировать роль',
                'main'          => 'Роли',
                'management'    => 'Управление доступом',
            ],
            'title' => 'Управление',
            'users' => [
                'all'               => 'Все пользователи',
                'change-password'   => 'Изменить пароль',
                'create'            => 'Создать пользователя',
                'deactivated'       => 'Заблокированные пользователи',
                'deleted'           => 'Удаленные пользователи',
                'edit'              => 'Редактирование учётной записи',
                'main'              => 'Пользователи',
                'view'              => 'Просмотр учётной записи',
            ],
            'page' => [
                'all'           => 'Все страницы',
                'create'        => 'Создать страницу',
                'edit'          => 'Редактировать страницу',
                'main'          => 'Страницы',
                'management'    => 'Управление страницами',
            ],
            'block' => [
                'all'           => 'Все блоки',
                'create'        => 'Создать блок',
                'edit'          => 'Редактировать блок',
                'main'          => 'Блоки',
                'management'    => 'Управление блоками',
            ],
            'news' => [
                'all'           => 'Все новости',
                'create'        => 'Создать новость',
                'edit'          => 'Редактировать роль',
                'main'          => 'Новости',
                'management'    => 'Управление новостями',
            ],
            'product' => [
                'all'        => 'Все продукты',
                'create'     => 'Создать продукт'
            ],
            'category' => [
                'all'           => 'Все категории',
                'create'        => 'Создать категорию',
                'create_root'     => 'Создать родительскую категорию',
                'edit'          => 'Редактировать категорию',
                'main'          => 'Категории',
                'management'    => 'Управление категориями',
            ],
            'collection' => [
                'all'           => 'Все коллекции',
                'create'        => 'Создать коллекцию',
                'edit'          => 'Редактировать коллекцию',
                'main'          => 'Коллекции',
                'management'    => 'Управление коллекциями',
            ],
            'marker' => [
                'all'           => 'Все маркеры',
                'create'        => 'Создать маркер',
                'edit'          => 'Редактировать маркер',
                'main'          => 'Маркеры',
                'management'    => 'Управление маркерами',
            ],
            'faq' => [
                'all'           => 'Все вопросы',
                'create'        => 'Создать вопрос',
                'edit'          => 'Редактировать вопрос',
                'main'          => 'Вопросы',
                'management'    => 'Управление вопросами',
            ],
            'zone' => [
                'all'           => 'Все зоны',
                'create'        => 'Создать зону',
                'edit'          => 'Редактировать зону',
                'main'          => 'Зоны',
                'management'    => 'Управление зонами',
            ],
            'good' => [
                'all'           => 'Все товары',
                'create'        => 'Создать товар',
                'edit'          => 'Редактировать товар',
                'main'          => 'Товары',
                'management'    => 'Управление товарами',
            ],
            'showroom' => [
                'all'           => 'Все посредники',
                'create'        => 'Создать посредника',
                'edit'          => 'Редактировать посредника',
                'main'          => 'Посредники',
                'management'    => 'Управление посредниками',
            ],
            'finishtissue' => [
                'all'           => 'Все ткани',
                'create'        => 'Создать ткань',
                'edit'          => 'Редактировать ткань',
                'main'          => 'Ткани',
                'management'    => 'Управление тканями',
            ],
            'popup' => [
                'edit'          => 'Редактировать объявление',
                'main'          => 'Объявление',
                'management'    => 'Управление объявлением',
            ],
            'settings' => [
                'edit'          => 'Редактировать настройки',
                'main'          => 'Настройки',
                'management'    => 'Управление настройками',
            ],
            'orders' => [
                'edit'          => 'Редактировать заказы',
                'main'          => 'Заказы',
                'management'    => 'Управление заказами'
            ],
            'baskets' => [
                'show'          => 'Показать корзину',
                'main'          => 'Корзины',
                'management'    => 'Управление корзинами'
            ],
            'templateMessage' => [
                'all'        => 'Все шаблоны',
                'create'     => 'Создать шаблон',
                'edit'       => 'Редактировать шаблон',
                'management' => 'Управление шаблонами сообщений',
                'main'       => 'Шаблоны',
            ],
            'documents' => [
                'all'        => 'Все документы',
                'create'     => 'Создать документ',
                'edit'       => 'Редактировать документ',
                'management' => 'Управление документыми',
                'main'       => 'Документы',
            ],
        ],
        'menu'    => [
            'content'   => 'Контент',
            'store'     => 'Магазин',
        ],
        'log-viewer'    => [
            'dashboard' => 'Обзор',
            'logs'      => 'Все записи',
            'main'      => 'Журнал ошибок',
        ],
        'sidebar'       => [
            'dashboard' => 'Системная панель',
            'general'   => 'Общие',
            'system'    => 'Система',
            'content'    => 'Содержимое',
        ],
    ],
    'language-picker'   => [
        'langs'     => [
            'ar'    => 'Арабский (Arabic)',
            'zh'    => '(Chinese Simplified)',
            'zh-TW' => '(Chinese Traditional)',
            'da'    => 'Датский (Danish)',
            'de'    => 'Немецкий (German)',
            'el'    => 'Греческий (Greek)',
            'en'    => 'Английский (English)',
            'es'    => 'Испанский (Spanish)',
            'fr'    => 'Французский (French)',
            'id'    => 'Индонезийский (Indonesian)',
            'it'    => 'Итальянский (Italian)',
            'ja'    => '(Japanese)',
            'nl'    => 'Голландский (Dutch)',
            'pt_BR' => 'Бразильский Португальский (Brazilian Portuguese)',
            'ru'    => 'Русский (Russian)',
            'sv'    => 'Шведский (Swedish)',
            'th'    => 'Тайский (Thai)',
            'tr'    => '(Turkish)',
        ],
        'language'  => 'Язык',
    ],
];
