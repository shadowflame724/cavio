                        ...
                        <div class="col-lg-10">
                            <select class="form-control" multiple="multiple"
                                ...
                                @foreach($collectionZones as $key => $collectionZone)
                                    <option value="{{$key}}"
                                            @foreach($good->collectionZone as $c) @if($key == $c->id)selected="selected"@endif @endforeach>{{ $collectionZone }}</option>
                                @endforeach
                            </select>
                        </div><!--col-lg-10-->
                    </div><!--form control-->
                        ...
                        <div class="col-lg-10">
                            <select class="form-control" multiple="multiple"
                                ...
                                @foreach($finishTissues as $key => $finishTissue)
                                    <option value="{{$key}}"
                                            @foreach($good->finishTissues as $f) @if($key == $f->id)selected="selected"@endif @endforeach>{{ $finishTissue }}</option>
                                @endforeach
                            </select>
                        </div><!--col-lg-10-->
                    </div><!--form control-->
...
@section('after-scripts')
    ...
    <script>
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            $('#edit-good').append('<input id=\"images' + i + '\" name=\"images[' + i + ']\" value=\"' + leafname + '\" type=\"hidden\"/>');
        }
        ...
        var myDropzone = new Dropzone(".dropzone", {
                ...
                success: function (file, res) {
                    ...
                    if (res['error']) {
                        ...
                        $('#edit-good').append('<input name=\"images[]\" value=\"' + res['success']['imgName'] + '\" type=\"hidden\"/>');
                        swal({
